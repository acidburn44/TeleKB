...
사용자가 **구독 중인 텔레그램 채널(기본) 및 옵션으로 그룹/슈퍼그룹**에서 새 메시지를 수집하여, **한국어가 아닌 메시지만 Gemini로 한국어 번역**하고, **메시지 1개당 Markdown 파일 1개**를 **단일 디렉토리**에 저장하는 로컬 앱을 만든다.
...
* 모든 Markdown 파일은 **단일 OUTPUT_DIR**에 저장한다(MUST).

### 6.2 파일명 규칙 (MUST)

* 포맷: `채널명_메시지첫문장_YYYYMMDD.md`
* “메시지첫문장” 정의: **첫 줄(개행 전까지)**
...
* Windows/macOS 공통으로 안전하게 저장되도록:
  ...
  * 길이 제한(권장, MUST로 구현해도 됨):
    ...
    * 채널명 최대 30자, 첫문장 최대 60자

### 6.5 이미지 처리 (MUST)

* 메시지에 포함된 이미지는 `OUTPUT_DIR/images` 디렉토리에 저장한다.
* 이미지 파일명은 중복 방지를 위해 `{channel_id}_{message_id}.jpg` 형식을 권장한다.
* Markdown 파일 내에 해당 이미지에 대한 링크를 포함시켜야 한다.
  * 예: `![Image](images/12345_67890.jpg)`
